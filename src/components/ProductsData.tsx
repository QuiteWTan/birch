    ...
    <Link href={{ pathname: "/product", query: { _id: item?._id } }}>
      <div className='w-full rounded-lg overflow-hidden'>
        ...
        <div className='border-[1px] border-slate-300 border-t-0 px-2 pt-2 pb-4 flex flex-col gap-y-1 bg-white rounded-b-lg'>
          <p className='text-sm md:text-base font-bold lg:text-lg font-urbanist'>{item?.title}</p>
          ...
          <div className='flex items-center justify-between'>
            <button onClick={ () => dispatch(addToCart(item) ) && toast.success(`${item?.title} added to cart`)} 
             className='bg-gray-700 text-white py-2 px-2 md:px-4 tracking-wide rounded-full hover:bg-black duration-500 transition-all text-[20px] md:text-[26px] flex items-center justify-center  gap-2'>
              <BiCartAdd className=""/>
